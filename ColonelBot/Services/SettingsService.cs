...
//Based off of RogueExceptions' DiscordBot example. 

namespace ColonelBot.Services
{
    class SettingsService
    {
            ...
            where SettingsT : class, new()
        {
            ...
            {
                string path = FileTools.BotDirectory + "/global.json";
                if (File.Exists(path))
            ...
            public async Task SaveGlobal(GlobalSettingsT settings)
            {
                while (true)
                    try
                    {
                        using (var fs = new FileStream(ColonelBot.FileTools.BotDirectory + "/global.json", FileMode.Create, FileAccess.Write, FileShare.None))
                ...
                where SettingsT : class, new()
            {
                ...
                public SettingsManager(string name)
                {
                    _dir = ColonelBot.FileTools.BotDirectory + "/Config/{name}";
                ...
                public Task Save(Server server, SettingsT settings) => Save(server.Id, settings);
                public Task Save(KeyValuePair<ulong, SettingsT> pair) => Save(pair.Key, pair.Value);
